...
CREATE TABLE IF NOT EXISTS `work` (
    ...
    `work_type`          VARCHAR(30)  NOT NULL               COMMENT '作品类型(TEXT/IMAGE/AUDIO/VIDEO/MODEL)',
...
CREATE TABLE IF NOT EXISTS `work_feature` (
    `id`                     BIGINT       NOT NULL               COMMENT '主键（雪花ID）',
    `work_id`                BIGINT       NOT NULL               COMMENT '作品ID',
    `work_type`              VARCHAR(30)  NOT NULL               COMMENT '作品类型(IMAGE/VIDEO/AUDIO/TEXT/MODEL)',
    `algo`                   VARCHAR(30)  NOT NULL               COMMENT '算法(PDQ/MINHASH/D2)',
    `algo_version`           VARCHAR(20)  NOT NULL               COMMENT '算法版本',
    `vector_dim`             INT          NOT NULL DEFAULT 256   COMMENT '向量维度（统一 256 bit）',
    `perceptual_hash`        VARCHAR(128) NOT NULL               COMMENT '256-bit 二进制指纹（hex 编码，64 字符）',
    `generation_fingerprint` VARCHAR(64)           DEFAULT NULL  COMMENT 'AIGC 来源指纹（model+prompt+seed 的 sha256）',
    ...
    `extra`                  JSON                  DEFAULT NULL  COMMENT '算法专属辅助信息（quality/frame_count/word_count 等）',
    `deleted_flag`           TINYINT      NOT NULL DEFAULT 0     COMMENT '删除标记 0-未删除 1-已删除',
    `created_at`             DATETIME     NOT NULL               COMMENT '创建时间(UTC)',
    `updated_at`             DATETIME     NOT NULL               COMMENT '更新时间(UTC)',
    PRIMARY KEY (`id`),
    KEY `idx_work_id` (`work_id`),
    KEY `idx_work_type_algo` (`work_type`, `algo`),
    KEY `idx_perceptual_hash` (`perceptual_hash`),
    KEY `idx_generation_fingerprint` (`generation_fingerprint`),
...
CREATE TABLE IF NOT EXISTS `work_similarity_check` (
    ...
    `algo`             VARCHAR(30)   NOT NULL               COMMENT '算法',
    `hamming_distance` INT           NOT NULL               COMMENT '汉明距离（0~256）',
    `similarity_score` DECIMAL(5,4)  NOT NULL               COMMENT '相似度分数（1 - distance/256）',
